...
export abstract class TicketRepository extends Repository<Ticket> {
  abstract findTickets(organizationId: string, params?: FindTicketsParams): Promise<Array<Ticket>>
  abstract count(organizationId: string, params?: FindTicketsParams): Promise<number>
  ...
  abstract atomicClaimOldestWaiting(serviceId: string, staffId: string): Promise<Ticket | null>
}
